          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-200">
              {isLoading ? (
                <tr>
                  <td colSpan={4} className="text-center py-6">
                    <div className="flex justify-center items-center">
                      <div className="animate-spin rounded-full h-8 w-8 md:h-16 md:w-16 border-t-4 border-b-4 border-blue-500"></div>
                    </div>
                  </td>
                </tr>
              ) : (
                posts.map((post) => (
                  <tr key={post.id}>
                    <td className="px-4 md:px-6 py-3 md:py-4 whitespace-nowrap overflow-hidden text-ellipsis max-w-[150px] md:max-w-[300px] text-sm font-medium text-gray-900">
                      {post.title}
                    </td>
                    ...
                    <td className="px-4 md:px-6 py-3 md:py-4 whitespace-nowrap text-sm text-gray-500 flex items-center space-x-2 md:space-x-4">
                      <select
                        value={String(post.status)}
                        onChange={(e) =>
                          handleStatusChange(post.id, Number(e.target.value))
                        }
                        className="px-2 py-1 text-sm border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600"
                      >
